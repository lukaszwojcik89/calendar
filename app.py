...
class Event:
    ...
    def start_time(self, val):
        ...
        try:
            ...
            raise ValueError(
                f'Invalid date format, use DD-MM-YYYY HH:MM, {val}')

        if parsed_datetime < datetime.now() + timedelta(minutes=15):
            raise ValueError(
                f'Try to use a date in the future, dummy... {parsed_datetime}')
    ...
    def __str__(self):
        ...
        hours, rest = total_seconds.seconds // (
            60 * 60), total_seconds.seconds % (60 * 60)
...
event = Event("Matura", "Kraków, Zacisze 8",
            "4-05-2022 9:00", 120, "Oliver", ["Ela", "Ola"])
print(event)
